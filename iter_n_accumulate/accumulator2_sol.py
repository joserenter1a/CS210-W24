...
def reduce_try_except(fn, iterable, initial_val=None):
    it = iter(iterable)
    if initial_val is None:
        try:
            initial_val = next(it)
        except StopIteration:
            raise TypeError("reduce() called on empty sequence")
    acc = initial_val
    for element in it:
        acc = fn(acc, element)
    return acc
...
if __name__ == '__main__':
    ...
    # Try without an initial val
    print(reduce_try_except(add_nums, [1,2,3]))